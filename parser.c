...
/**
 * copy_the_characters - duplicates characters
 ...
 */
char *copy_the_characters(char *pathstr, int start, int stop)
{
...
/**
 * search_command_path - finds this cmd in the PATH string
 * @info: the info struct
 * @pathstr: the PATH string
 * @cmd: the cmd to find
 *
...
char *search_command_path(info_t *info, char *pathstr, char *cmd)
{
	...
	while (1)
	{
		...
		{
			path = copy_the_characters(pathstr, curr_pos, i);
			if (!*path)
				_strcat(path, cmd);
			else
			{
